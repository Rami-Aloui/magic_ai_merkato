...
class UserController extends Controller
{
    ...
    public function openAIGeneratorWorkbook($slug)
    {
        ...
        if (view()->exists('panel.admin.custom.user.openai.generator_workbook')) {
            $view = 'panel.admin.custom.user.openai.generator_workbook';
        } else {
    ...
    public function subscriptionPlans()
    {
        ...
        if (view()->exists('panel.admin.custom.user.payment.subscriptionPlans')) {
            $view = 'panel.admin.custom.user.payment.subscriptionPlans';
        } else {
            $view = 'panel.user.finance.subscriptionPlans';
        }

        return view($view, compact('plans', 'openAiList', 'prepaidplans', 'is_active_gateway', 'activeGateways', 'activesubid'));
    }
    ...
    public function merkadoAll()
    {
        $visibilitycharts='display:none';
        $visibilitybuton='';
        
        return view('panel.user.openai.merkado')->with(['visibilitycharts' => $visibilitycharts,'visibilitybuton' => $visibilitybuton]);
    }
    public function merkadoCola()
    {
        $visibilitybuton='display:none';
        $visibilitycharts='';
